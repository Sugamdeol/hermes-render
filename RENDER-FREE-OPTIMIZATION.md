...
3. **`HERMES_FAILOVER=0` is now the default.** If you ever run a second
   ...
   coordinate.
4. **~~CRITICAL and EMERGENCY have not been observed firing.~~ Now observed —
   see §10.1.** The whole ladder has been seen end to end under real pressure.
   What remains unproven is narrower: the pressure was generated by a synthetic
   allocator holding pages inside the cgroup, not by 400 MB of genuine
   conversation state, so the *stages* are proven while the exact usage that
   would reach them in production is still a model.
...
### 10.1 The whole degradation ladder, observed end to end

The earlier run only ever reached HIGH, because the workload peaked near 220 MB
of 512. To exercise CRITICAL and EMERGENCY I ran the same real `bootstrap.sh`
stack in the same 512 MB / 0.1 CPU cgroup and added **real resident pressure
inside the cgroup** — a process that allocates and touches pages 15 MB at a
time, holding them, rather than shrinking `memory.max` to fake a percentage.
The guard therefore reacted to genuine `memory.current`, exactly as it would in
production.

The complete transcript, verbatim:

```
memory guard: NORMAL -> WATCH at 363MB/512MB (70%); gateway=91MB dashboard=64MB children=364MB procs=29 threads=45
memory guard: WATCH -> HIGH at 423MB/512MB (82%); gateway=91MB dashboard=64MB children=409MB procs=29 threads=45
memory guard: HIGH -> CRITICAL at 453MB/512MB (88%); gateway=91MB dashboard=64MB children=439MB procs=29 threads=45
memory guard: CRITICAL -> EMERGENCY at 483MB/512MB (94%); gateway=91MB dashboard=64MB children=473MB procs=31 threads=47
memory guard EMERGENCY: 94% >= 92% (3 samples) — recycling the dashboard to free web-chat agents; watchdog will restart it
warning: dashboard process not found; restarting (1/10)
starting dashboard on 0.0.0.0:10000 (supervised)
memory guard: EMERGENCY -> NORMAL at 151MB/512MB (29%); gateway=91MB dashboard=63MB children=138MB procs=29 threads=45
```

All five stages fired in order, at the thresholds `env/common.env` declares
(70 / 80 / 87 / 92), and the guard recovered on its own once pressure was
released — no restart of the container, no operator intervention.

**The invariant that matters held.** Read straight out of `/proc` afterwards,
as root:

```
GATEWAY   pid=337131  rss=91MB  oom_adj=-500    <- same PID before, during and after
DASHBOARD pid=345720  rss=63MB  oom_adj= 250    <- recycled: was 337236
```

The gateway's PID never changed across the entire excursion. EMERGENCY did
exactly what it is designed to do — it took the **dashboard**, freed its
web-chat agents, and the watchdog brought it back (`1/10`, well inside the
restart budget) — while the process carrying the conversation was never
touched. `/health` answered `200` in **2.0 ms** afterwards, and the cgroup
settled back to **131 MB of 512**.

Two caveats, stated plainly rather than buried:

- The pressure came from a synthetic allocator holding pages, not from 400 MB
  of genuine conversation state. The *stages* are proven to fire and recover;
  the precise real-world workload that would reach 88% is still a model.
- A liveness check I ran mid-verification reported the gateway as dead. It was
  wrong: `kill -0` run as the unprivileged `user` against a uid-999 process
  fails on **permissions**, not existence. Re-checked as root, the process was
  alive the whole time. Worth recording because the same check would produce a
  false alarm in any harness that does not run as root.
...
NORMAL, driven by real resident pressure inside the cgroup rather than a
shrunken cap. EMERGENCY recycled the dashboard and the watchdog restarted it,
while the gateway's PID and its `oom_adj=-500` were unchanged throughout.
