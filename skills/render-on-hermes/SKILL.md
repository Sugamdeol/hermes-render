...
is configured (`GIT_STATE_REPO` for the git backend), do not tell the user that
sessions, memories, dashboard configuration, logs, or files saved there will
persist. With one configured, state is restored at boot and saved again within
seconds of any change from the private GitHub repo. Provider keys, channel
tokens, and other important configuration must still live in Render environment
variables, not only in `/opt/data/.env`. The state branch includes everything
under `/opt/data` — `.env` included, in plaintext mode — so the repository must
be treated as sensitive; `.env` can be age-encrypted (`GIT_STATE_ENV_MODE`)
or left out of the backup entirely.
