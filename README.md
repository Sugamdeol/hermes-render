...
What the Chat tab gives you:

- **Conversation sidebar** grouped by Pinned / Today / Yesterday / last 7 days / older, with full-text search (`Ctrl+K`), pin, archive, rename (double-click the title), export and delete from a per-row menu. The list is served by the plugin's `/sessions` endpoint (ordered by most recent activity, sub-agent `tool` sessions hidden) and includes conversations started from Telegram, the CLI or any other platform.
- **Instant, read-only history.** Clicking a conversation loads its transcript straight from the session database (`/transcript/{id}`) without building an agent, so browsing old chats is fast and costs no RAM. The conversation is resumed through `session.resume` only when you send the next message (or press **Resume now**).
- **Chronological tool steps.** Each Hermes turn shows its tool calls in the order they ran, inside the turn that issued them, with argument preview, duration, result summary and expandable output — both live (streamed `tool.start`/`tool.complete` events) and for saved history (rebuilt from the stored `tool_calls`/`tool` rows). Diffs from file edits and `todo` lists render inline.
- **Approvals and questions in the thread.** Dangerous-command approvals (`approval.request`), clarifying questions, and sudo/secret prompts appear as cards with Allow once / for session / always / Deny buttons instead of hanging until the gateway times out.
- **Steer while running.** Typing while Hermes is working sends `session.steer`, which lands with the next tool result without interrupting the turn; **Stop** (or `Esc`) interrupts.
- Mode / model / toolset pickers, temporary chats, drag-and-drop attachments, branching, Markdown/JSON export, read-only share links, a details panel (`Ctrl+Shift+I`) with model, context-window usage and token counts, and automatic WebSocket reconnect.
