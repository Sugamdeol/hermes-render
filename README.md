...
| **Chat** tab missing from the sidebar, or chat won't connect ("gateway WebSocket failed" / 4403) | The bundled `hermes-chat-dashboard` plugin serves the Chat tab and talks to `/api/ws`; upstream answers that WebSocket with 4403 unless `HERMES_DASHBOARD_TUI=1` (and the tab override only renders when embedded chat is enabled, so it is invisible when set to `0`). The Blueprint sets it to `1`; if you forked an older copy or set it to `0`, restore it and restart. The plugin itself must also be present in `$HERMES_HOME/plugins/hermes-chat-dashboard/` — bootstrap installs it from the image on every boot. If you disabled `HERMES_DASHBOARD`, the tab cannot exist. |
