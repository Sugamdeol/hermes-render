...
| **Chat** tab missing from the sidebar, `/chat` redirects back to **Sessions**, or chat won't connect ("gateway WebSocket failed" / 4403) | The bundled `hermes-chat-dashboard` plugin serves the Chat tab and talks to `/api/ws`; upstream answers that WebSocket with 4403 unless `HERMES_DASHBOARD_TUI=1` (and the tab override only renders when embedded chat is enabled, so with it set to `0` the route falls through and the dashboard bounces `/chat` back to **Sessions**). The Blueprint sets it to `1`; if you forked an older copy, copied an outdated `.env.example`, or set it to `0`, restore it and restart. The plugin itself must also be present in `$HERMES_HOME/plugins/hermes-chat-dashboard/` — bootstrap installs it from the image on every boot. If you disabled `HERMES_DASHBOARD`, the tab cannot exist. |
